...
def predict(features, use_pca=False):
    try:
        ...
        # Konversi ke persen dan tambahkan simbol %
        class_probs = {
            label_map[i]: f"{round(float(probabilities[i]) * 100, 2)}%"
...
page = st.sidebar.radio("Silahkan pilih menu :", ["🏠 Beranda", "🖼️ Input Gambar", "🧪 Try Sample"])
